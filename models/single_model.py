...
class SingleModel(BaseModel):
    ...
    def __init__(self, opt):
        # 用于保存判别器的输出结果张量的计数器
        self.save_count = 0
        # self.real_patch = 0  # 应该是一个张量 不应该这么定义
        # self.fake_patch = 0
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'D_P']
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'D_P']
        ...
        if self.isTrain:
            ...
            # itertools.chain函数将参数连接在一起，使得优化器可以同时更新三个判别器的参数
    ...
    def backward_D_P(self):
        """计算局部判别器 D_P 的 GAN 损失"""
        loss_D_P = self.backward_D_basic(self.netD_P, self.real_patch, self.fake_patch)
        if self.opt.patchD_3 > 0:
            for i in range(self.opt.patchD_3):
                loss_D_P += self.backward_D_basic(self.netD_P, self.real_patch_1[i], self.fake_patch_1[i])
        ...
        # self.loss_D_P.backward()   # 与D_A和D_B不同，是否需要修改？
    ...
    def optimize_parameters(self):
        ...
        self.set_requires_grad([self.netD_A, self.netD_B, self.netD_P], False)  # Ds require no gradients when optimizing Gs
        ...
        self.set_requires_grad([self.netD_A, self.netD_B, self.netD_P], True)
        self.optimizer_D.zero_grad()  # 将 D_A 和 D_B 的梯度设为零
        self.backward_D_A()  # 计算 D_A 的梯度
        self.backward_D_B()  # 计算 D_B 的梯度
        # D_P
        self.optimizer_D_P.zero_grad()  # 梯度归零
        self.backward_D_P()  # 计算 D_P 的梯度
        self.optimizer_D.step()  # 更新 D_A、D_B 的权重
        self.optimizer_D_P.step()  # 更新 D_P的权重
